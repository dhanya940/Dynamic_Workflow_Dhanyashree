...
function navBar(){nav.innerHTML=token()?`<a href="#home">Home</a><a href="#forms">Forms</a><button onclick="logout()">Sign out</button>`:`<a href="#signin">Sign in</a><a href="#signup">Sign up</a>`}
...
function renderForms(fs){document.getElementById("formList").innerHTML=fs.length?fs.map(f=>`<div class="card formcard"><div class="between"><h2>${esc(f.title)}</h2><span class="badge ${f.status}">${f.status}</span></div><p class="muted">Updated ${new Date(f.updated_at).toLocaleString()}</p><a class="btn" href="#builder/${f.id}">Open</a></div>`).join(""):`<div class="card empty">No forms yet. Create your first form.</div>`}
...
function fieldEditor(f={}){let opts=(f.options||[]).map(o=>`${o.option_label}|${o.option_value}`).join("\n");return `<label>Label</label><input id="flabel" class="input" value="${esc(f.label||"")}"><label>Type</label><select id="ftype" class="input">${["text","email","number","date","dropdown","checkbox","file","textarea"].map(x=>`<option ${f.field_type===x?"selected":""}>${x}</option>`).join("")}</select><label>Placeholder</label><input id="fph" class="input" value="${esc(f.placeholder||"")}"><label><input id="freq" type="checkbox" ${f.is_required?"checked":""}> Required</label><label>Validation JSON (optional)</label><input id="fval" class="input" placeholder='{"min":0,"max":100}' value='${esc(f.validation_config?JSON.stringify(f.validation_config):"")}'> <label>Options (one <code>label|value</code> per line)</label><textarea id="fopts" class="input" rows="4">${esc(opts)}</textarea><div class="actions"><button class="btn primary" onclick="${f.id?`saveField('${f.id}')`:"saveNewField()"}">Save</button><button class="btn" onclick="this.closest('.card').remove()">Cancel</button></div>`}
...
function newRule(){let fs=currentForm.fields||[];if(fs.length<2)return toast("Add at least two fields first");app.querySelector(".container").insertAdjacentHTML("afterbegin",`<div class="card"><h2>Add conditional rule</h2><label>When field</label><select id="rtrigger" class="input">${fs.map(f=>`<option value="${f.id}">${esc(f.label)}</option>`).join("")}</select><label>Operator</label><select id="rop" class="input"><option value="equals">equals</option><option value="not_equals">not equals</option><option value="contains">contains</option></select><label>Value</label><input id="rvalue" class="input"><label>Action</label><select id="raction" class="input"><option value="show">show</option><option value="hide">hide</option></select><label>Target field</label><select id="rtarget" class="input">${fs.map(f=>`<option value="${f.id}">${esc(f.label)}</option>`).join("")}</select><button class="btn primary" onclick="saveRule()">Save rule</button></div>`)}
async function saveRule(){try{await api("/forms/"+currentForm.id+"/rules",{method:"POST",body:JSON.stringify({trigger_field_id:rtrigger.value,operator:rop.value,comparison_value:rvalue.value,target_field_id:rtarget.value,action:raction.value})});toast("Rule added");await builder(currentForm.id);await loadRules()}catch(e){err(e)}}
